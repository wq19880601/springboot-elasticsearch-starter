 ...
 * @author Kevin Leturc
 */
public class ElasticsearchQuery<K> implements SimpleQuery<ElasticsearchQuery<K>>, Fetchable<K> {
    ...
    public ElasticsearchQuery(Client client, Function<SearchHit, K> transformer, ElasticsearchSerializer serializer, String index, String type, String routing) {
        this.queryMixin = new QueryMixin<ElasticsearchQuery<K>>(this, new DefaultQueryMetadata().noValidate(), false);
        this.client = client;
        this.transformer = transformer;
        this.serializer = serializer;

        esConfig = new EsConfig(index, type, Optional.ofNullable(routing));
    }
    ...
    public long fetchCount() {
        ...
        SearchRequestBuilder sb = client.prepareSearch(getIndex())
            .setTypes(getType())
            .setSearchType(SearchType.DFS_QUERY_THEN_FETCH).setQuery(createQuery(filter))
            .setSize(0);
        sb.setIndicesOptions(IndicesOptions.lenientExpandOpen());

        return sb.get().getHits().getTotalHits();
    }
    ...
    private SearchResponse executeSearch(String index, String type, Predicate filter,
        ...
        requestBuilder.setIndicesOptions(IndicesOptions.lenientExpandOpen());

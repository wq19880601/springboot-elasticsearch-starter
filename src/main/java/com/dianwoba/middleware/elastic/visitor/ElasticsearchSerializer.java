 ...
 */
package com.dianwoba.middleware.elastic.visitor;
...
public class ElasticsearchSerializer implements Visitor<Object, BoolQueryBuilder> {
    ...
    public Object visit(Constant<?> expr, @Nullable BoolQueryBuilder context) {
        ...
        } else if (Date.class.isAssignableFrom(expr.getType())) {
            Object constant = expr.getConstant();
            String date = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(constant);
            return date;
        } else {
    ...
    public Object visit(Operation<?> expr, @Nullable BoolQueryBuilder context) {
        ...
        if (op == Ops.EQ) {
            ...
            if (keyArg instanceof Path<?> && isIdPath((Path<?>) expr.getArg(0))) {
                ...
                return QueryBuilders.termQuery(asDBKey(expr, 0), value);
            }
            ...
            // Currently all queries are made with ignore case sensitive
            String value = StringUtils.toString(asDBValue(expr, 1));
            return QueryBuilders.matchPhrasePrefixQuery(asDBKey(expr, 0), value);
            ...
            return QueryBuilders.boolQuery().mustNot(QueryBuilders.existsQuery(asDBKey(expr, 0)));
    ...
    protected String getKeyForPath(Path<?> expr, PathMetadata metadata) {
        if (isIdPath(expr)) {
            ...
            String pathKey = metadata.getElement().toString();
            return CaseFormat.LOWER_CAMEL.to(CaseFormat.LOWER_UNDERSCORE, pathKey);
        }
    }
